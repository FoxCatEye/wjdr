...
def train():
    ...
    if exists(Template(r"icon\tpl1719198082012.png", threshold=0.5, record_pos=(-0.44, -0.783), resolution=(414, 780))):
        ...
        print_space('\033[31m未找到对应图案\033[0m')
    print_space('训练完成')
    time.sleep(1)
    touch([14, 823])
    time.sleep(1)  # 等待1秒
...
def Production_soldiers():
    ...
    if exists(Template(r"icon\tpl1719478488282.png",rgb= True, threshold=0.9, record_pos=(-0.186, -0.058),
        ...
        train()
    if exists(Template(r"icon\tpl1719480722195.png",rgb=True, threshold=0.85, record_pos=(-0.437, 0.046), resolution=(414, 780))):
        print_space("跳转到矛兵兵营...")
...
def re_connet():
    if exists(Template(r"icon\tpl1720766916047.png", rgb=False, record_pos=(-0.168, -0.088), resolution=(1080, 1920))):
        print('\033[31m在其他设备登录，等待5分钟后重新连接\033[0m')
        time.sleep(300)
        try:
            print_space('点击重新连接')
            re = 1
...
#pyinstaller  -w  --onefile --name "无尽冬日" --icon "E:\测试文件\测试工具\版本控制\Wjdr\icon\tpl1719196072757.png" --add-data "E:\测试文件\测试工具\AirtestIDE\airtest:airtest" --add-data "E:\测试文件\测试工具\版本控制\Wjdr\icon:icon" --add-data "E:\测试文件\测试工具\版本控制\Wjdr\wjdr.py:." E:\测试文件\测试工具\版本控制\Wjdr\windows.py
