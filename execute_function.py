...
def start_exe():
    while True:
        try:
            ...
            itemData_1 = settings.value(
                "模拟器地址",
                os.path.join("E:", "leidian", "LDPlayer9", "dnplayer.exe"),
                type=str,
            )
            subprocess.Popen(itemData_1)
            print("启动成功")
            break
        except:
...
def start_app():
    ...
    while True:
        try:
            ...
            touch(
                Template(
                    r"icon/tpl1719196072757.png",
                    record_pos=(0.112, -0.519),
                    resolution=(414, 780),
                )
            )
...
def start_simple(button_start_id):  # 模拟器启动相关
    if button_start_id == 1:
        ...
        # start_exe()
        """启动模拟器线程"""
        start_exe_thread = threading.Thread(
            target=start_exe
        ...
        # start_app()
        """启动app线程"""
        start_app_thread = threading.Thread(
            target=start_app
        ...
        while True:
            ...
            if self.checkBox_donate.isChecked() and now.hour % 2 and now.minute == 1:
                try:
                    ...
                    print("程序执行异常，结束该任务，执行其他任务")
            if (
                self.checkBox_recruit.isChecked()
                and now.hour == 1
                and now.minute % 5 == 0
            ):
                try:
            ...
            if (
                ...
                try:
                    ...
                    print("程序执行异常，结束该任务，执行其他任务")
            if (
                self.checkBox_Treasure_Chest.isChecked()
                and now.minute == 49
                and now.second % 20 == 0
            ):
                try:
        ...
        while execute:
            try:
                ...
                if stop_event.is_set():
                    ...
                    self.simple_stop_button()  # 停止后按钮变为开始
                    break
                mining_collision_time1 = int(
                    settings.value("攻击检测设置", 20, type=str)
                )
